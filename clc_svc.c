...
static void *
_inicializa_random_1 (int  *argp, struct svc_req *rqstp)
{
	return (inicializa_random_1_svc(*argp, rqstp));
}

static int *
_obtiene_1er_random_1 (void  *argp, struct svc_req *rqstp)
{
	return (obtiene_1er_random_1_svc(rqstp));
}

static int *
_obtiene_2do_random_1 (void  *argp, struct svc_req *rqstp)
{
	return (obtiene_2do_random_1_svc(rqstp));
}
...
static void
clc_prog_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union {
		int inicializa_random_1_arg;
	...
	case INICIALIZA_RANDOM:
		...
		local = (char *(*)(char *, struct svc_req *)) _inicializa_random_1;
		break;

	case OBTIENE_1ER_RANDOM:
		_xdr_argument = (xdrproc_t) xdr_void;
		_xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _obtiene_1er_random_1;
		break;

	case OBTIENE_2DO_RANDOM:
		_xdr_argument = (xdrproc_t) xdr_void;
		_xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _obtiene_2do_random_1;
		break;
...
main (int argc, char **argv)
{
	...
	if (!svc_register(transp, CLC_PROG, CLC_VERS, clc_prog_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (CLC_PROG, CLC_VERS, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, CLC_PROG, CLC_VERS, clc_prog_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (CLC_PROG, CLC_VERS, tcp).");
		exit(1);
	}
